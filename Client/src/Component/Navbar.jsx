...
const Navbar = () => {
	...
	const handleToggleView = () => {
		if (listView === true || listView === 'true') {
			...
			localStorage.setItem('currentView', false);
		} else {
	...
	const handleSearchFunc = () => {
		...
		Array.from(NoteArray).forEach((note) => {
			let content = note.getElementsByClassName('noteContent')[0].innerText;
			if (input.value === '') {
				note.classList.remove('d-none');
			} else if (content.toLowerCase().includes(input)) {
				note.classList.remove('d-none');
			} else {
	...
	return (
		<header>
			<a href='https://quicknote.onrender.com/pwainstall' target='_blank'>Open</a>
